...
This connector allows an IAM system to Create, Read, Update, and Delete information from Grafana Open Source or Grafana Enterprise systems for automated provisioning of Users, Organizations and Data Sources.
...
<table>
  ...
  <tr>
   ...
   <td>Designed to allow the provisioning system to update a Grafana Dashboard. (Not Implemented)
   </td>
  </tr>
  <tr>
   <td>IO Error Retries
  ...
  <tr>
   ...
   <td>When a dashboard template is supplied the connector will attempt to create a new Grafana Dashboard for an organization containing the uid of the newly created Datasource. This is done through substitution of any string sequence “&lt;DataSourceUID>” with the actual UID of the newly created Datasource.  
   </td>
  </tr>
</table>
...
There is no particular schema for grafana dashboards except that the data is
...
# Importing Grouper Groups and Downstream Provisioning of Those Groups as Grafana Groups with Loki Datasource
...
_                               theUid = basic.getAttributeValue(shadow, 'uid')_

_                                return ((null == theUid) || ("" == theUid))_
